...
for epsilon in sp.arange(0.1, 1.7, 0.05):
#for epsilon in sp.arange(0.1, 0.3, 0.005):
	a = CHMC(nWalkers=10, nD=50, epsilon=epsilon, nSteps_per_samp=50)
	...
	data = sp.zeros((a.nD, (steps - burnin + 1)*a.nWalkers))
	...
	diagz = sp.diag(sp.diag(err))
	#plt.subplot(211)
	#plt.imshow(LA.inv(a.log_pX.inv_cov))
	#plt.subplot(212)
	#plt.imshow(cov)
	#plt.show()
	print (epsilon, sp.mean(err - diagz))
	plt.scatter(epsilon, sp.mean(err - diagz))
plt.yscale('log')
plt.show()
